...
{
    if (argc < 3) {
        printf("complex_filter <input> <output>");
        return -1;
    }

    const char * in_filename = argv[1];
    const char * out_filename = argv[2];
    const char * complex_filter_descr = "[0:v]split[v1][v2];[v1]scale=320:-1:flags=lanczos,vflip[s];[v2][s]overlay=10:10";
    ...
    if (avfilter_graph_parse2(filter_graph, complex_filter_descr, &inputs, &outputs) < 0) {
        fprintf(stderr, "avfilter_graph_parse2()\n");
        return -1;
    }

    for (auto ptr = inputs; ptr; ptr = ptr->next) {
        if (avfilter_link(in_filter_ctx, 0, ptr->filter_ctx, ptr->pad_idx) != 0) {
            fprintf(stderr, "avfilter_link()\n");
            return -1;
        }
    }

    for (auto ptr = outputs; ptr; ptr = ptr->next) {
        if (avfilter_link(ptr->filter_ctx, ptr->pad_idx, out_filter_ctx, 0) != 0) {
            fprintf(stderr, "avfilter_link()\n");
            return -1;
        }
    }

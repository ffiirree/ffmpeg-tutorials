...
int main(int argc, char *argv[])
{
    ...
    args::parser parser("Hardware accelerated transcoding", false);
    ...
    // create hardware device for AV_HWDEVICE_TYPE_CUDA
    ...
    decoder_fmt_ctx->video_codec =
        avcodec_find_decoder_by_name(parser.get<std::string>("decoder", "h264_cuvid").c_str());
    ...
    CHECK(avcodec_parameters_to_context(decoder_ctx,
                                        decoder_fmt_ctx->streams[video_stream_idx]->codecpar) >= 0);
    ...
    while (av_read_frame(decoder_fmt_ctx, in_packet) >= 0) {
        ...
        while (ret >= 0) {
            ...
            if (!initialized) {
                ...
                CHECK(avcodec_parameters_from_context(encoder_fmt_ctx->streams[0]->codecpar, encoder_ctx) >=
                      0);
                ...
                LOG(INFO) << fmt::format(
                    ...
                    av_get_pix_fmt_name(encoder_ctx->pix_fmt), encoder_ctx->framerate.num,
                    encoder_ctx->framerate.den, encoder_ctx->time_base.num, encoder_ctx->time_base.den,
            ...
            in_frame->pts =
                av_rescale_q(in_frame->pts, decoder_fmt_ctx->streams[video_stream_idx]->time_base,
    ...
    LOG(INFO) << fmt::format("decoded frames: {}, encoded frames: {}", decoder_ctx->frame_number,
